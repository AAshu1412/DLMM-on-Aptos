...
module token11 {
    ...
    fun init_module(admin: &signer) {
        ...
        move_to(
            admin,
            DLT1Capabilities { mint_cap, burn_cap, freeze_cap }
        );
    ...
    /// Burn DLT1 tokens
    public entry fun burn_tokens(admin: &signer, amount: u64) acquires DLT1Capabilities, TokenConfig {
        let admin_addr = signer::address_of(admin);
        assert!(exists<DLT1Capabilities>(admin_addr), E_NOT_ADMIN);
    ...
    public fun mint_rewards(
        admin_addr: address, to: address, amount: u64
    ...
    // fun test_mint_rewards(account: signer) acquires DLT1Capabilities, TokenConfig {
    //     init_module(&account);
    //     mint_tokens(&account, @ashu_address, 1000000000000000000);
    //     let (cu, mu, su) = get_token_info();
    //     print(&cu);
    //     print(&mu);
    //     print(&su);
...
module dlmm {
    ...
    struct LiquidatorData has store, drop {
        token1_amount: u64,
        token2_amount: u64,
        active_bin_id: u64,
        liquidity_share: u64
    ...
    public entry fun initialize_factory(admin: &signer) {
        ...
        assert!(
            !exists<Factory>(admin_addr),
            error::already_exists(E_POOL_ALREADY_EXISTS)
        );
    ...
    public entry fun add_user_tokens(
        user: &signer,
        token1_address: address,
        token2_address: address,
        token3_address: address,
        ...
        move_to(user, user_token);

    }


    // Helper function to deduct a single token amount
fun deduct_single_token(
    user_tokens: &mut UserTokens,
    token_address: address,
    amount: u64
): bool {
    if (user_tokens.token1_address == token_address) {
        if (user_tokens.token1_amount >= amount) {
            user_tokens.token1_amount = user_tokens.token1_amount - amount;
            return true
        } else {
            return false
        }
    } else if (user_tokens.token2_address == token_address) {
        if (user_tokens.token2_amount >= amount) {
            user_tokens.token2_amount = user_tokens.token2_amount - amount;
            return true
        } else {
            return false
        }
    } else if (user_tokens.token3_address == token_address) {
        if (user_tokens.token3_amount >= amount) {
            user_tokens.token3_amount = user_tokens.token3_amount - amount;
            return true
        } else {
            return false
        }
...
fun add_single_token(
    ...
    public entry fun create_pool(
        ...
        bin_step: u16,
        fees: u64
    ) acquires Factory, LiquidatorPosition, UserTokens {
        ...
        assert!(
            ...
            error::invalid_argument(E_INVALID_TOKEN_PAIR)
        );
        assert!(
            initial_token1_amount > 0 && initial_token2_amount > 0,
            error::invalid_argument(E_INSUFFICIENT_AMOUNT)
        );
        ...
        // aptos_account::transfer_coin<token1_metadata>(creator, pool_address, initial_token1_amount);
        // aptos_account::transfer_coin<token2_metadata>(creator, pool_address, initial_token2_amount);
                    let userr_token = borrow_global_mut<UserTokens>(creator_addr);
                    deduct_single_token( userr_token, token1_address, initial_token1_amount);
                    deduct_single_token( userr_token, token2_address, initial_token2_amount);
        ...
        if (!exists<LiquidatorPosition>(creator_addr)) {
            let liquidator_position = LiquidatorPosition { pools: simple_map::create() };
    ...
    public entry fun add_liquidity(
        ...
        token2_amount: u64,
        target_bin_id: u64
    ) acquires Factory, Pool, LiquidatorPosition , UserTokens{
        ...
        assert!(
            simple_map::contains_key(&factory.pools, &pool_key),
            error::not_found(E_POOL_NOT_EXISTS)
        );
        ...
        // aptos_account::transfer_coin<pool.token1_metadata>(liquidator, pool_address, token1_amount);
        // aptos_account::transfer_coin<pool.token2_metadata>(liquidator, pool_address, token2_amount);

        let userr_token = borrow_global_mut<UserTokens>(liquidator_addr);
        add_single_token( userr_token, token1_address, token1_amount);
        add_single_token( userr_token, token2_address, token2_amount);
        // primary_fungible_store::transfer(
        //     liquidator,
        //     pool.token1_metadata,
        //     pool_address,
        //     token1_amount
        // );
        // primary_fungible_store::transfer(
        //     liquidator,
        //     pool.token2_metadata,
        //     pool_address,
        //     token2_amount
        // );
        ...
        if (simple_map::contains_key(&liquidator_position.pools, &pool_key)) {
            let liquidator_data =
                simple_map::borrow_mut(&mut liquidator_position.pools, &pool_key);
            liquidator_data.token1_amount = liquidator_data.token1_amount
                + token1_amount;
            liquidator_data.token2_amount = liquidator_data.token2_amount
                + token2_amount;
            liquidator_data.liquidity_share =
                liquidator_data.liquidity_share + token1_amount + token2_amount;
        } else {
            let liquidator_data = LiquidatorData {
                token1_amount,
                token2_amount,
    ...
    public fun get_pool_info(
        token1_address: address, token2_address: address, bin_step: u16
    ...
    public fun get_liquidator_position(
        ...
        (
            liquidator_data.token1_amount,
            liquidator_data.token2_amount,
            liquidator_data.active_bin_id,
            liquidator_data.liquidity_share
        )
    }
    ...
    public entry fun swap(
    ...
    ) acquires Factory, Pool ,UserTokens{
        let user_addr = signer::address_of(user);
        ...
        // Get pool
        let factory = borrow_global<Factory>(@ashu_address);
        let (token1_std, token2_std) =
            standardize_token_pair(token_in_address, token_out_address);
        ...
        let (amount_out, fees_collected, bins_crossed) =
        ...
        assert!(
            amount_out >= min_amount_out, error::invalid_argument(E_SLIPPAGE_EXCEEDED)
        );

        // Get token metadata
        // let token_in_metadata =
        //     if (swap_token1_for_token2) pool.token1_metadata
        //     else pool.token2_metadata;
        // let token_out_metadata =
        //     if (swap_token1_for_token2) pool.token2_metadata
        //     else pool.token1_metadata;

        // Transfer tokens
        // aptos_account::transfer(user, token_in_metadata, amount_in);
        // aptos_account::transfer(user, token_out_metadata, amount_out);
        // let pool_constructor_ref = object::create_object(@ashu_address);
        // let pool_signer = object::generate_signer(&pool_constructor_ref);
        let userr_token = borrow_global_mut<UserTokens>(user_addr);
        deduct_single_token( userr_token, token_in_address, amount_in);
        add_single_token( userr_token, token_out_address, amount_out);
    ...
    fun calculate_swap_amount(
        ...
        while (remaining_amount_in > 0) {
            ...
            let available_liquidity =
                if (swap_token1_for_token2) bin.token2_amount
                else bin.token1_amount;
            ...
            let (bin_amount_out, bin_amount_in_used, bin_fees) =
                calculate_bin_swap(
                    remaining_amount_in,
                    available_liquidity,
                    bin_price,
    ...
    fun calculate_bin_price(bin_id: u64, bin_step: u16): u64 {
        ...
        if (bin_id >= middle_bin) {
            ...
            base_price * power(price_multiplier, bin_difference)
                / power(10000u64, bin_difference)
        } else {
            let price_divisor = 10000u64 + (bin_step as u64);
            let bin_difference = middle_bin - bin_id;
            base_price * power(10000u64, bin_difference)
                / power(price_divisor, bin_difference)
        }
    }
    ...
    fun calculate_bin_swap(
        ...
        let amount_out =
            if (swap_token1_for_token2) {
                ...
                let amount_out_before_limit = (amount_in_after_fees * bin_price)
                    / 1000000u64;
                if (amount_out_before_limit > available_liquidity) available_liquidity
                else amount_out_before_limit
            } else {
                // Buying token1 with token2
                let amount_out_before_limit = (amount_in_after_fees * 1000000u64)
                    / bin_price;
                if (amount_out_before_limit > available_liquidity) available_liquidity
                else amount_out_before_limit
            };
        ...
        let actual_fee_amount =
            (actual_amount_in_used * total_fee_bps) / (10000u64 - total_fee_bps);
    ...
    public entry fun remove_liquidity(
        ...
        // Validate inputs
        assert!(
            liquidity_percentage > 0 && liquidity_percentage <= 100,
            error::invalid_argument(E_INSUFFICIENT_AMOUNT)
        );
        ...
        assert!(
            exists<LiquidatorPosition>(liquidator_addr),
            error::not_found(E_NO_LIQUIDITY_TO_REMOVE)
        );
        let liquidator_position = borrow_global_mut<LiquidatorPosition>(liquidator_addr);
        assert!(
            simple_map::contains_key(&liquidator_position.pools, &pool_key),
            error::not_found(E_NO_LIQUIDITY_TO_REMOVE)
        );
        ...
        let liquidator_data =
            simple_map::borrow_mut(&mut liquidator_position.pools, &pool_key);
        assert!(
            liquidator_data.liquidity_share > 0,
            error::invalid_state(E_NO_LIQUIDITY_TO_REMOVE)
        );

        // Check if bin exists and liquidator has position in it
        assert!(
            simple_map::contains_key(&pool.all_bins, &target_bin_id),
            error::not_found(E_POOL_NOT_EXISTS)
        );
        let bin = simple_map::borrow_mut(&mut pool.all_bins, &target_bin_id);
        assert!(
            vector::contains(&bin.liquidator_addresses, &liquidator_addr),
            error::permission_denied(E_NO_LIQUIDITY_TO_REMOVE)
        );

        // Calculate liquidity share in this bin
        let total_bin_liquidity = bin.total_liquidity;
        let liquidator_bin_share =
            calculate_liquidator_bin_share(liquidator_addr, bin, liquidator_data);

        // Calculate amounts to remove based on percentage
        let liquidity_to_remove = (liquidator_bin_share * liquidity_percentage) / 100;
        let token1_to_remove =
            (bin.token1_amount * liquidity_to_remove) / total_bin_liquidity;
        let token2_to_remove =
            (bin.token2_amount * liquidity_to_remove) / total_bin_liquidity;

        assert!(
            token1_to_remove > 0 || token2_to_remove > 0,
        ...
        // Update liquidator position
        let liquidity_share_to_burn =
            (liquidator_data.liquidity_share * liquidity_percentage) / 100;
        liquidator_data.token1_amount = liquidator_data.token1_amount
            - token1_to_remove;
        liquidator_data.token2_amount = liquidator_data.token2_amount
            - token2_to_remove;
        liquidator_data.liquidity_share =
            liquidator_data.liquidity_share - liquidity_share_to_burn;

        // Remove liquidator from bin if no more liquidity
        if (liquidity_percentage == 100) {
            let (_, index) = vector::index_of(
                &bin.liquidator_addresses, &liquidator_addr
            );
            ...
            if (total_position_liquidity == 0) {
                let (_, pool_index) = vector::index_of(
                    &pool.liquidators, &liquidator_addr
                );
        ...
        if (token1_to_remove > 0) {
            ...
            // let pool_signer = object::generate_signer(&pool_constructor_ref);
            let userr_token = borrow_global_mut<UserTokens>(liquidator_addr);
            add_single_token( userr_token, token1_address, token1_to_remove);
            // primary_fungible_store::transfer(
            //     &pool_signer,
            //     pool.token1_metadata,
            //     liquidator_addr,
        ...
        if (token2_to_remove > 0) {
            ...
            // let pool_signer = object::generate_signer(&pool_constructor_ref);
            let userr_token = borrow_global_mut<UserTokens>(liquidator_addr);
            add_single_token( userr_token, token2_address, token2_to_remove);
            // primary_fungible_store::transfer(
            //     &pool_signer,
            //     pool.token2_metadata,
            //     liquidator_addr,
    ...
    fun calculate_liquidator_bin_share(
        liquidator_addr: address, bin: &Bin, liquidator_data: &LiquidatorData
    ): u64 {
